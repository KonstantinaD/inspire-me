...
public class ArticleResourceAssembler implements ResourceAssembler<Article, Resource<Article>> {
    ...
    public Resource<Article> toResource(Article article) {
        ...
        return new Resource<>(article,
                linkTo(methodOn(ArticleController.class).getArticle(article)).withSelfRel(),
                linkTo(methodOn(ArticleController.class).getAllArticlesByCategory(article.getCategory())).withRel("articles/category/{categoryId}"),
